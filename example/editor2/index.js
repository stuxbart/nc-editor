...
const textCode = `import { Range } from '../selection';
...
function makeDocument(linesCount) {
	...
	editor.setSelection(new nc.Selection(0, 0, 0, 0));
	editor.insert('');
	editor.disableTokenization();
	editor.disableEditEvent();
	editor.disableLinesChangedEvent();
	editor.disableSelectionsUpdates();

	const textLength = 20;
	const inserts = Math.floor(linesCount / textLength);
	const m = Math.round(inserts / 100);
	for (let i = 0; i < inserts; i++) {
		// setTimeout to keep page interactive
		setTimeout(() => {
			editor.insert(text20lines + '\n');
			if (i % m === 0) {
				editor.enableEditEvent();
				editor.enableLinesChangedEvent();
				editor.insert('');
				editor.disableEditEvent();
				editor.disableLinesChangedEvent();
			}
	...
	setTimeout(() => {
		editor.enableEditEvent();
		editor.enableLinesChangedEvent();
		editor.enableSelectionsUpdates();
		const leftLines = linesCount % textLength;
		const lines = text20lines.split('\n').slice(0, leftLines);
